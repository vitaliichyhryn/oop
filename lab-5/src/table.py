...
@Gtk.Template(resource_path="/com/chyhryn/Lab5/gtk/table.ui")
class Table(Adw.Window):
    __gtype_name__: str = "Table"
    
    column_view: Gtk.ColumnView = Gtk.Template.Child()
    
    entry_removed = GObject.Signal("entry_removed")
    ...
    def update_table(self, table: list[dict[str, str]]) -> None:
        self.table = table
        ...
        self.single_selection.set_model(list_store)
    
    @Gtk.Template.Callback()
    def delete_entry(self, button) -> None:
        entry_pos = self.single_selection.get_selected()
        if entry_pos == Gtk.INVALID_LIST_POSITION:
            return
        self.table.pop(entry_pos + 1)
        self.update_table(self.table)
        self.entry_removed.emit()
